package VirtualMemoryVorgabecode.osbsp;
...
public class OperatingSystem {
	...
	public synchronized int read(int pid, int virtAdr) {
		...
		PageTableEntry pageTableentry = process.pageTable.getPte(getVirtualPageNum(virtAdr));


		//Seite nicht vorhanden
		if (pageTableentry == null) {
			System.out.println("Seite nicht vorhanden");
		} else {
			// Seite vorhanden: Seite im RAM vorhanden?
			if (!pageTableentry.valid) {
				// Seite auf Platte --> Seitenfehler
				pageTableentry = handlePageFault(pageTableentry, pid);
			}
			realeAdresse = pageTableentry.realPageFrameAdr + getOffset(virtAdr);
			datenwort = readFromRAM(realeAdresse);
	...
	private int getVirtualPageNum(int virtAdr) {
		return Math.floorDiv(virtAdr, getPAGE_SIZE());
	}
